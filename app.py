...
st.set_page_config(page_title="Influencer Detector", page_icon="📊", layout="centered")
...
def convert_to_numeric(value):
    if isinstance(value, str):
        ...
        if 'k' in v:
            ...
            return float(v.replace('b', '')) * 1000000000
    try:
        return float(value)
...
if st.session_state.model is None:
    ...
    if st.button("Check Influencer"):
        ...
        classes = st.session_state.model.classes_.tolist()
        ...
        if pred[0] == 1:
            confidence = float(prob[classes.index(1)]) if 1 in classes else 1.0
            st.success("Good Influencer!")
            st.progress(confidence)
            st.caption(f"Confidence: {confidence * 100:.1f}%")
        else:
            confidence = float(prob[classes.index(0)]) if 0 in classes else 1.0
